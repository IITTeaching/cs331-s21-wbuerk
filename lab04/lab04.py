...
class ConstrainedList (list):
    ...
    def __getitem__(self, idx):
        if idx < 0 or idx >= len(self):
            raise ValueError(
                'Can only use positive, valid indexes on constrained lists!')
    ...
    def __getattribute__(self, name):
        if name in ('insert', 'min', 'max', 'append', 'extend'
            ...
            raise AttributeError('Method "' + name +
                                 '" not supported on constrained list!')
        else:
    ...
    def __str__(self):
        raise AttributeError(
            'Constrained lists do not support stringification!')
...
class ArrayList:
    def __init__(self, n=0):
        ...
        # the attribute self.len should be record the length of the list (do not rename!)
        self.len = n
    ...
    def __str__(self):
        ...
        string = "["
        for x in range(0, self.len-1):
            string += f"{self.data[x]}, "
        string += f"{self.data[self.len-1]}]"
        return string
        # END SOLUTION
    ...
    def remove(self, value):
        ...
        for i in range(self.len):
            if self.data[i] == value:
                ...
                break
        if idx == -1:
            raise ValueError("Value must be in list")
    ...
    def index(self, value, i=0, j=None):
        ...
        # BEGIN SOLUTION
        if j == None:
            for idx in range(i, self.len):
                if self.data[idx] == value:
                    return idx
        elif j < 0:
            for idx in range(i, self.len+j):
                if self.data[idx] == value:
                    return idx
        else:
    ...
    def __add__(self, other):
        ...
        for x in range(self.len):
            newa.data[x] = self.data[x]
        for x in range(other.len):
            newa.data[self.len+x] = other.data[x]
    ...
    def copy(self):
        ...
        # BEGIN SOLUTION
        newArrayList = ArrayList(self.len)

        for x in range(self.len):
            newArrayList.data[x] = self.data[x]
...
def test_case_6():
    ...
    tc.assertEqual(list(range(10))+list(range(10, 0, -1)) +
                   data, arrayListToList(lst))
    suc()
